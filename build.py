...
---------
    ...
    your own root location using '--custom-root-path'. Any textbook-root directory
    should contain the following files and folders in the below structure:

    Content Directory: 'textbook/content/'
    ======================================
    This is the main directory for storing website content.

    In the top level of the directory (i.e. 'textbook/content/'), you can add markdown
    page files (which author the webpages), Jupyter Python notebooks, and local images
    in an 'images/' subdirectory.

    You can also create subdirectories to define "sections" or chapters of your website
    (e.g. 'textbook/content/05_erps/'). The "sections" have their title (as displayed on
    the website) defined in a special 'README.md' file inside the section
    ...
    for example 'textbook/content/05_erps/02_hnn_core.md'. Each markdown page file will
    eventually be built into an output HTML file where the numerical prefix is removed,
    and the filetype is HTML; continuing our example,
    ...
    Finally, 'textbook/content/' should also contain an 'assets/' subdirectory which
    contains your CSS (filename 'styles.css') and static icon assets.

    All other files inside 'textbook/content/' will output files which are generated by
    the build system, either as a result of notebook execution or website building. This
    includes '*.html' files, '*.json' files, and 'output_nb_<notebook name>/'
    subdirectories. HTML files can be safely deleted and then regenerated on website
    building. JSON and 'output_nb_*' files *can* technically be deleted and regenerated
    if you must, but are generally kept and tracked on git, and *should* generally not
    be deleted. In particular, JSON files store the most recent execution history of
    each notebook (which we track to decide whether a notebook has been updated), and
    'output_nb_*' tends to store heavy image assets which tend *not* to change between
    hnn-core code versions. In other words, you should feel free to delete HTML files
    (which can do using `make clean`) but should refrain from deleting JSON and
    'output_nb_*' files.
    ...
    directory as the file used to create them under the 'textbook/content/' directory.
    ...
    of output and comparison with 'textbook/content/', and can be safely deleted.
    ...
    3. **Execute Jupyter notebooks** (via `execute_and_convert_nbs_to_json`):
       - Scan 'textbook/content' directory for .ipynb files
    ...
    4. **Generate HTML pages** (via `generate_page_html`):
       - Scan 'textbook/content' directory for numbered markdown files (##_*.md)
    ...
    Notes
    -----
    - You can mix and match all values of '--code-version' and '--execution-type' arguments.
